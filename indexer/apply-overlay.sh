...
# Applies repo overlay files onto /etc/wazuh-indexer
...
src_dir="${repo_root}/indexer/overlay"

dst_dir="/etc/wazuh-indexer"

restart_unit() {
  if command -v systemctl >/dev/null 2>&1; then
    if systemctl status soc-indexer.service >/dev/null 2>&1; then
      systemctl restart soc-indexer.service
      return 0
    fi
    echo "Missing systemd alias: soc-indexer.service" >&2
...
echo "Applied indexer overlay to ${dst_dir}"

echo "Restarting service (soc-indexer if available)..."
restart_unit
echo "You can check: systemctl status soc-indexer"
